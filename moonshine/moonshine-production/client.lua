	...
	while true do
		...
		if start then
			if production < 3 then
				DrawScriptText(vector3(-36.83, 3030.73, 41.02), '~g~E ~w~· Start productie!')
				if IsControlJustReleased(0, Keys["E"]) then
					...
					ESX.TriggerServerCallback('bamboozle-production:GetInventory', function (boiler, bucket, corn, barley, yeast, water, cb, source)
						if boiler < 1 then
							exports['mythic_notify']:DoHudText('error', '[INFO] Je hebt geen ketel')
							start = true
						else
							exports['mythic_notify']:DoHudText('success', '[INFO] Ketel aan het plaatsen')
							...
							exports['progressBars']:startUI((2500), "Ketel aan het plaatsen...")
		...
		if boilerplaced then
			if production < 3 then
				DrawScriptText(vector3(-36.83, 3030.73, 41.02), '~g~E ~w~· Voeg water toe!')
				if IsControlJustReleased(0, Keys["E"]) then
					DisableControlAction(0, 38, true)
					boilerplaced = false
					...
					ESX.TriggerServerCallback('bamboozle-production:GetInventory', function (boiler, bucket, corn, barley, yeast, water, cb, source)
						if water < 1 then
							exports['mythic_notify']:DoHudText('error', '[INFO] Je hebt geen water')
							boilerplaced = true
						else
							exports['mythic_notify']:DoHudText('success', '[INFO] Water aan het ingieten')
							...
							exports['progressBars']:startUI((5000), "Ingieten...")
		...
		if wateradded then
			if production < 3 then
				DrawScriptText(vector3(-36.83, 3030.73, 41.02), '~g~E ~w~· Water opstoken!')
				if IsControlJustReleased(0, Keys["E"]) then
					DisableControlAction(0, 38, true)
					wateradded = false
					
					exports['mythic_notify']:DoHudText('success', '[INFO] Water aan het opstoken')
					...
					exports['progressBars']:startUI((5000), "Opstoken...")
		...
		if vuur then
			if production < 3 then
				DrawScriptText(vector3(-36.83, 3030.73, 41.02), '~g~E ~w~· Voeg maisvlokken toe!')
				if IsControlJustReleased(0, Keys["E"]) then
					DisableControlAction(0, 38, true)
					vuur = false
					...
					ESX.TriggerServerCallback('bamboozle-production:GetInventory', function (boiler, bucket, corn, barley, yeast, water, cb, source)
						if corn < 1 then
							exports['mythic_notify']:DoHudText('error', '[INFO] Je hebt geen maisvlokken')
							vuur = true
						else
							exports['mythic_notify']:DoHudText('success', '[INFO] Maisvlokken toe aan het voegen')
							...
							exports['progressBars']:startUI((5000), "Toevoegen...")
		...
		if mais then
			if production < 3 then
				DrawScriptText(vector3(-36.83, 3030.73, 41.02), '~g~E ~w~· Voeg gerst toe!')
				if IsControlJustReleased(0, Keys["E"]) then
					DisableControlAction(0, 38, true)
					mais = false
					...
					ESX.TriggerServerCallback('bamboozle-production:GetInventory', function (boiler, bucket, corn, barley, yeast, water, cb, source)
						if barley < 1 then
							exports['mythic_notify']:DoHudText('error', '[INFO] Je hebt geen gerst')
							mais = true
						else
							exports['mythic_notify']:DoHudText('success', '[INFO] Gerst toe aan het voegen')
		...
		if gerst then
			if production < 3 then
				DrawScriptText(vector3(-36.83, 3030.73, 41.02), '~g~E ~w~· Voeg gist toe!')
				if IsControlJustReleased(0, Keys["E"]) then
					DisableControlAction(0, 38, true)
					gerst = false
					...
					ESX.TriggerServerCallback('bamboozle-production:GetInventory', function (boiler, bucket, corn, barley, yeast, water, cb, source)
						if yeast < 1 then
							exports['mythic_notify']:DoHudText('error', '[INFO] Je hebt geen gist')
							gerst = true
						else
							exports['mythic_notify']:DoHudText('success', '[INFO] Gist toe aan het voegen')
		...
		if gist then
			if production < 3 then
				DrawScriptText(vector3(-36.83, 3030.73, 41.02), '~g~E ~w~· Roeren!')
				if IsControlJustReleased(0, Keys["E"]) then
					DisableControlAction(0, 38, true)
					gist = false

					exports['mythic_notify']:DoHudText('success', '[INFO] Aan het roeren')
					...
					exports['progressBars']:startUI((5000), "Roeren...")
		...
		if roeren then
			if production < 3 then
				DrawScriptText(vector3(-36.83, 3030.73, 41.02), '~g~E ~w~ Schenken!')
				if IsControlJustReleased(0, Keys["E"]) then
					DisableControlAction(0, 38, true)
					roeren = false

					exports['mythic_notify']:DoHudText('success', '[INFO] Aan het schenken')
					...
					exports['progressBars']:startUI((5000), "Schenken...")
