...
Citizen.CreateThread(function()
	while true do
		...
		if start then
			...
			if inkoop < 3 then
				DrawScriptText(vector3(2436.45, 4976.77, 46.57), '~g~E ~w~· Om spullen in te kopen!')
				if IsControlJustReleased(0, Keys["E"]) then
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cocaina_laptop', {
							...
							align = "top-left",
							elements = {
								{label = ("Ketel Kopen"), value = "buy_ketel"},
								{label = ("Moonshine Starterpakket"), value = "buy_items"},
						}}, function(data, menu)
	...
	while true do
		...
		if moonshinecar then
			if uitpakken < 3 and vehModel == GetHashKey("speedo4") then
				...
				DrawScriptText(vector3(-26.73, 3026.16, 40.96), '~g~E ~w~· Om spullen uit te laden!')
				if IsControlJustReleased(0, Keys["E"]) then
					DisableControlAction(0, 38, true)
					
					exports['mythic_notify']:DoHudText('success', '[SUCCESS] Voertuig wordt uitgeladen!')
					...
					exports['mythic_notify']:DoHudText('success', '[SUCCESS] Voertuig is uitgeladen!')
					moonshinecar = false
...
AddEventHandler('bamboozle-collecting:startinkoop', function()
	...
	MissionText("Pak de ~r~auto ~w~langs het gebouw en rijd naar de locatie om de productie te starten", 20000)
end)
...
Citizen.CreateThread(function ()
    ...
    AddTextComponentString('Moonshine: Inkoop')
