...
AddEventHandler('bamboozle-selling:SellMoonshine', function(sellprice, value)
	...
	if moonshine >= tonumber(value) then 
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'je hebt '.. value .. ' Moonshine verkocht voor ' .. price .. ' zwartgeld!'})
		sendtoDiscord(_source, xPlayer.identifier, sellprice, value, drug, price)
	else 
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Je hebt niet genoeg zakjes flesjes!'})
	end
end)
...
function sendtoDiscord(id, player, price, value, drug, totalprice)
    ...
    local WebHook = ""
		...
		local discordInfo = {
			...
			["description"] = player .. " (" .. spelernaam .. ") Heeft " .. value .. 'x ' .. drug .. ' verkocht voor '.. price ..' euro **PER STUK** en ' .. totalprice .. ' in totaal',
			["footer"] = {
